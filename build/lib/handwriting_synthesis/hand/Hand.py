...
from handwriting_synthesis.hand._draw import _draw2
...
class Hand(object):
    ...
    def getXML(self, lines, biases = None, styles = None, stroke_colors=None, stroke_widths=None):
        valid_char_set = set(drawing.alphabet)
        for line_num, line in enumerate(lines):
            if len(line) > 75:
                raise ValueError(
        ...
        strokes = self._sample(lines, biases=biases, styles=styles)
        result = _draw2(strokes, lines, stroke_colors=stroke_colors, stroke_widths=stroke_widths)
        return result

    def GetCharacterSet(self):
        return drawing.alphabet
